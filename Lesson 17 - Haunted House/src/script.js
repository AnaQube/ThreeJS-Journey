...
/**
 * 1. Set PCFSoftShadowMap on renderer.shadowMap.type for better shadows.
 * 2. Optimize shadowMaps by doing shadow.mapSize.set(x, y) and setting the near and far clips for the light cameras
 * 3. Set castShadow and receiveShadow for objects that cast/receive shadows
 * 4. Set wrapS and wrapT to RepeatMapping when you need textures to repeat
 * 5. Set uv2 attribute when you use AOMaps
 */
...
const bushMaterial = new THREE.MeshStandardMaterial({ color: '#88ff38' })
...
for (let i = 0; i < 50; i++) {
    // Angle is the distribution of graves around a circle
    const angle = Math.random() * Math.PI * 2
    // Radius is the distance from the house where 3 is the min distance and 6 is the range of distances
 ...
 * Ghosts
 */
const ghost1 = new THREE.PointLight('#2222ff', 2, 3)
const ghost2 = new THREE.PointLight('#ff7733', 2, 3)
const ghost3 = new THREE.PointLight('#22ff76', 2, 3)
...
const tick = () =>
{
    ...
    ghost1.position.x = Math.cos(ghostAngle) * 6
    ghost1.position.z = Math.sin(ghostAngle) * 6
    ghost1.position.y = Math.cos(ghostAngle * 3)
    ...
    ghost3.position.y = Math.cos(ghost2Angle * 4) + Math.sin(elapsedTime * 2.5)
